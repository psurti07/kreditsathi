                ...
                success: function (result) {
                    ...
                    if (result.type === 'SUCCESS') {
                        if (result.redirectUrl != '') {
                            ...
                            var redirect = `{{ route('loan.agent.loan.details') }}`;
                            toastr.success(`OTP verified successfully. Redirecting in <span id="countdown">3</span> seconds automatically`, '', { timeOut: 0, extendedTimeOut: 0 });
                            function updateCountdown(count) {
                                document.getElementById('countdown').textContent = count;
                            }
                            let count = 3;
